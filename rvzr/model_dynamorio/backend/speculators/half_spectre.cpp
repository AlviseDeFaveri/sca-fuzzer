///
/// File: Implementation of the Half-Spectre Speculator
///
...
#include "speculators/half_spectre.hpp"
...
static std::pair<reg_id_t, byte *> get_load_destination(void *dc, byte *pc, Decoder &decoder)
{
...
bool SpeculatorHalfSpectre::handle_exception(void *dc, dr_siginfo_t *siginfo)
{
    // Use superclass implementation if we shouldn't handle this exception
    if (skip_speculation())
    ...
    if (not controlled_loads.contains(mc->pc))
        return SpeculatorABC::handle_exception(dc, siginfo);

    // Decode the instruction and return its destination if it's a load
    const auto [dest_reg, next_pc] = get_load_destination(dc, mc->pc, decoder);

    // Inject the poison value
